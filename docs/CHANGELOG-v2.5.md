...
- 区分“取消已赚快照”和“自然跨午夜”：取消历史已赚会删除对应联动记录，跨午夜不会误删昨日记录，也不会回填用户未启用期间的数据。
- 已赚日均改为只按已生成记录的日期计算；今日实时金额只进入“已赚”显示，不稀释日均。
- 批量生成已赚快照时带入摸鱼/加班 session；跨天 session 按日期裁剪后分别进入两天的净工时统计。
- 补全 2026 年中秋、国庆假期与调休上班日；工作时间外默认记录为“加班”，工时段内默认“摸鱼”。
- 新增“下班后首次打开仍同步今日记录”的组件回归测试。
