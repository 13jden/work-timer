...
/** 2026 年法定节假日
 *
 * 2026 年中秋节：9月25日（周五）— 9月27日（周日），共3天
 * 2026 年国庆节：10月1日（周四）— 10月7日（周三），共7天
 * 调休上班：9月20日（周日）、10月10日（周六）→ 不列在 HOLIDAYS，保持为普通周末
 */
...
/**
 * v2.5-patch16 T-531 · 调休上班日（强制工作日，覆盖周末默认休息）
 *
 * 2026 年中秋节调休上班：9 月 20 日（周日）
 * 2026 年国庆节调休上班：10 月 10 日（周六）
 *
 * 优先级（isWorkday 中）：
 *   1. override         （用户手动标记优先）
 *   2. HOLIDAYS         （法定节假日 → 休息）
 *   3. MAKEUP_WORKDAYS  （调休上班 → 工作，覆盖周末默认休息）
 *   4. restMode / hourly-daily 默认
 */
